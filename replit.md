...
GrowthTrack is a React Native Expo mobile application designed for teen health tracking with parent oversight capabilities. The app enables teen athletes to log health metrics (sleep, workouts, nutrition, daily check-ins), receive personalized recommendations, and optionally connect with parents who can monitor their progress through a separate dashboard. The app supports Apple HealthKit integration for automatic health data syncing on iOS devices.
...
- **Styling**: StyleSheet-based component styling with consistent design tokens (colors: primary green #10B981, blue #3B82F6)

### Route Structure
The app uses role-based navigation flows:
- `/(auth)` - Login and registration screens
- `/(teen-onboarding)` - 6-step teen onboarding flow (age, goals, sports, availability, health connection, parent linking)
- `/(parent-onboarding)` - Parent onboarding (invite code generation, supervision level, guardrails)
- `/(tabs)` - Main app tabs for general users
- `/(teen-app)` - Teen-specific features (home, scoliosis care, logging, insights, profile)
- `/(parent-app)` - Parent dashboard (overview, trends, alerts, settings)
...
- **Parent Components** (`components/parent/`): Dashboard widgets (TeenStatus, WeeklySummary, AtRiskIndicators, GuardrailsStatus)
...
- User roles: 'teen' | 'parent' | 'admin'
- Profile types: TeenProfile, ParentProfile with goals, sports, availability
...
- Endpoints for auth, profiles, logging, recommendations, parent oversight
